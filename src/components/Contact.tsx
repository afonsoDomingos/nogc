...
export default function Contact() {
  ...
  const handleWhatsAppSend = (e: React.FormEvent) => {
    ...
    const greeting = language === "en" 
      ? `Hello NOGC,${lineBreak}I would like to get in touch. Here are my details:`
      : `Olá NOGC,${lineBreak}Gostaria de entrar em contacto. Aqui estão os meus detalhes:`;

    const nameLabel = language === "en" ? "*Name:*" : "*Nome:*";
    const emailLabel = language === "en" ? "*Email:*" : "*E-mail:*";
    const subjectLabel = language === "en" ? "*Subject:*" : "*Assunto:*";
    const messageLabel = language === "en" ? "*Message:*" : "*Mensagem:*";

    let messageText = `${greeting}${lineBreak}${lineBreak}`;
    messageText += `${nameLabel} ${form.name}${lineBreak}`;
    messageText += `${emailLabel} ${form.email}${lineBreak}`;
    if (form.subject) {
      messageText += `${subjectLabel} ${form.subject}${lineBreak}`;
    }
    messageText += `${messageLabel} ${form.message}`;

    const encodedText = encodeURIComponent(messageText);
    const whatsappUrl = `https://wa.me/258843019001?text=${encodedText}`;
    ...
    try {
      ...
      console.error("Failed to log contact to database", err);
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-4 pt-2">
                  ...
                  <button
                    ...
                    disabled={status === "submitting" || status === "success"}
                    className="w-full md:w-auto px-8 py-4 rounded-full font-semibold text-sm tracking-wide text-white bg-gradient-to-r from-emerald-600 to-green-700 hover:from-emerald-500 hover:to-green-600 transition-all duration-300 border border-white/10 shadow-lg hover:shadow-emerald-500/20 flex items-center justify-center gap-2 group cursor-pointer disabled:opacity-50"
                  >
                    <span>{language === "en" ? "Send via WhatsApp" : "Enviar via WhatsApp"}</span>
                    <svg className="w-4.5 h-4.5 fill-white group-hover:scale-110 transition-transform" viewBox="0 0 24 24">
